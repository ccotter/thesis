...
\begin{figure}[t]
\includegraphics[scale=.6]{memory.jpg}
\caption{Data structure relationships associated with object-based reverse
mapping. The associated logic consists of over 1900 lines of C
code.~\cite{objectrmap}}
\label{fig:memory}
\end{figure}
...
Figure~\ref{fig:memory} demonstrates the complicated data structures associated
...
processes are not restricted to blocking {\tt dput()}
...
exploit optimal concurrency in Determinator's design. Aviram et al. demonstrate
a case where running parallel
{\tt make -j2}~\cite{Aviram10} might be blocked on thread one while. Applications that use the non-blocking syscalls
