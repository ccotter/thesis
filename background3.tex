...
We would also like to point out the importance of repeatability in simulators.
...
parents and children. The kernel ``provides no file systems, writable shared
...
three-way diff. The kernel copies bytes
from the child that have changed since the previous {\tt Snap} invocation into
the parent. Bytes that changed in the parent but not the child are not copied.
...
providing applications with a fork-join model, where a process forks many
children to perform some computation and simply {\tt Merge} the results back
into itself.
